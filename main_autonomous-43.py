...
async def scan(app=None):
    ...
    # ── DEBUG: collect rejection reasons ────────────────────────────────────
    passed, skipped = [], []
    reject_ev      = 0
    reject_kelly   = 0
    reject_conf    = 0
    ...
    for p in raw:
        ...
        if ok:
            ...
            # Tally reason for the debug report
            if "odds" in reason:    reject_odds  += 1
            elif "conf" in reason:  reject_conf  += 1
            elif "EV" in reason:    reject_ev    += 1
            elif "Kelly" in reason: reject_kelly += 1
            skipped.append((p, reason))
            logger.info(f"  ✗ {p['match']} | odds={p['odds']} ev={p['ev']:+.3f} → {reason}")

    logger.info(f"Passed: {len(passed)}  Skipped: {len(skipped)}")

    # ── EV distribution of first 10 raw picks ───────────────────────────────
    if raw:
        ev_sample = "\n".join(
            f"  [{i+1:02d}] odds={p['odds']:.2f}  EV={p['ev']:+.3f}  "
            f"Kelly={p['kelly']*100:.1f}%  conf={p['conf']:.0%}  {p['match'][:30]}"
            for i, p in enumerate(raw[:10])
        )
        logger.info(f"First 10 raw picks (EV sample):\n{ev_sample}")

    # ── Odds distribution ────────────────────────────────────────────────────
    if raw:
        odds_vals = [p["odds"] for p in raw]
        logger.info(
            f"Odds range: min={min(odds_vals):.2f}  max={max(odds_vals):.2f}  "
            f"median={sorted(odds_vals)[len(odds_vals)//2]:.2f}"
        )
        ev_pos = sum(1 for p in raw if p["ev"] > 0.05)
        logger.info(f"Picks with EV>0.05: {ev_pos}/{len(raw)}")

    if not passed:
        lines = [
            f"🔍 Scan complete: 0 picks passed filters",
            f"Total matches fetched : {len(raw)}",
            f"Analyzed              : {len(raw)}",
            f"",
            f"Rejection breakdown:",
            f"  ✗ Odds outside 1.80–5.00 : {reject_odds}",
            f"  ✗ Confidence too low      : {reject_conf}",
            f"  ✗ EV below threshold      : {reject_ev}",
            f"  ✗ Kelly below threshold   : {reject_kelly}",
            f"",
            f"Bank: {_bank:.0f} UAH",
            f"Tip: check LOG for per-pick breakdown",
        ]
        msg = "\n".join(lines)
        if app and CHAT_ID:
    ...
    header = (
        ...
        f"Fetched: {len(raw)} | Passed: {len(passed)} | Filtered: {len(skipped)}\n"
        f"Odds:{reject_odds} Conf:{reject_conf} EV:{reject_ev} Kelly:{reject_kelly} rejected\n"
        f"Bank: {_bank:.0f} UAH"
    )
    if app and CHAT_ID:
