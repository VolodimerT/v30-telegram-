...
async def scan():
    ...
    try:
        ...
        for idx, pick in enumerate(enriched_picks):
            ...
            # FILTER 1: ACCEPT or RECONSIDER (bootstrap mode until 200+ settled bets)
            if recommendation not in ("ACCEPT", "RECONSIDER"):
                logger.info(f"  ❌ Skip recommendation={recommendation}")
                filtered += 1
                continue

            # FILTER 2: confidence >= 62%
            if conf < MIN_CONFIDENCE:
                logger.info(f"  ❌ Skip confidence {conf:.0%} < {MIN_CONFIDENCE:.0%}")
                filtered += 1
                continue

            # FILTER 3: odds range 1.80-5.00
            ...
            emoji = "✅"  # Always ACCEPT at this point
            
            logger.info(f"  ✅ SENDING ACCEPT: Kelly={kelly_fraction*100:.1f}% Stake={optimal_stake:.0f} UAH")
