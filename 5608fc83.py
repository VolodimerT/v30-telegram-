...
def estimate_model_prob(best_odds, avg_odds, book_count, market, selection):
    ...
    if market == "h2h":
        bonus = min(gap * 7.5, 2.5)
        consensus = 1.4 if book_count >= 6 else 1.0 if book_count >= 4 else 0.4 if book_count >= 3 else 0.0
        prob = implied_best + bonus + consensus + 0.5
        if profile == "draw":
            prob -= 1.8
...
def class_from_ev(ev_calibrated, ci_low, data_quality, odds, market, selection):
    ...
    if profile == "underdog_long":
        return "MICRO" if ev_calibrated >= 9 and ci_low >= 1.0 and data_quality != "LOW" else "PASS"
    if profile == "draw":
        return "MICRO" if ev_calibrated >= 7 and ci_low >= 1.0 else "PASS"
    if market == "spreads":
...
def finalize_candidate(candidate, request_data, run_id):
    ...
    elif profile == "underdog_long" and data_quality != "HIGH":
