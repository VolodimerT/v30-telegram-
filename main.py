...
def estimate_model_prob(best_odds, avg_odds, book_count, market, selection):
    ...
    price_gap = max(0.0, best_odds - avg_odds)
    gap_bonus = min(price_gap * 7.0, 2.2)
    consensus_bonus = 1.2 if book_count >= 6 else 0.8 if book_count >= 4 else 0.3 if book_count >= 3 else 0.0
    market_bonus = 0.4 if market == "h2h" else 0.0
    prob = implied_best + gap_bonus + consensus_bonus + market_bonus
    low_sel = selection.lower()
    if market == "h2h" and "draw" in low_sel:
        prob -= 2.5
    if best_odds >= 3.0:
        prob -= min((best_odds - 3.0) * 2.2, 5.5)
    if best_odds >= 4.5:
        prob -= 2.0
    return round(max(30.0, min(prob, 72.0)), 2)
...
def calibration_factor(book_count, odds_age_minutes, lineup_confirmed, market, data_quality, odds, selection):
    ...
    if odds >= 4.5:
        factor -= 0.18
    if market == "h2h" and "draw" in selection.lower():
        factor -= 0.12
    return round(max(factor, 0.15), 2)
...
def calc_ci_low(ev_calibrated, book_count, odds_age_minutes, variance, data_quality, odds, market, selection):
    ...
    if data_quality == "LOW":
        penalty += 3.0
    if odds >= 3.0:
        penalty += min((odds - 3.0) * 1.4, 4.0)
    if market == "h2h" and "draw" in selection.lower():
        penalty += 2.5
    return round(ev_calibrated - penalty, 2)

def class_from_ev(ev_calibrated, ci_low, data_quality, odds):
    if ci_low < 0:
        return "PASS"
    if odds >= 4.5:
        return "MICRO" if ev_calibrated >= 9 and ci_low >= 1 else "PASS"
...
def finalize_candidate(candidate, request_data, run_id):
    ...
    if candidate["commence_time"] < now:
        ...
        decision = class_from_ev(ev_calibrated, ci_low, data_quality, candidate["odds_best"])
        ...
        if candidate["market"] == "h2h" and "draw" in candidate["selection"].lower() and decision == "CORE":
            decision = "SUPPORT"
            reasons.append("DRAW_CAP")
        if candidate["odds_best"] >= 4.5 and CLASS_ORDER[decision] > CLASS_ORDER["MICRO"]:
            decision = "MICRO"
        ...
        if decision == "PASS":
            reasons.append("RULES_BLOCK" if not reasons else reasons[-1])
        else:
...
def build_scorecard(candidate, implied, ev_raw, ev_calibrated, ci_low, data_quality):
    ...
    score -= 2 if candidate["odds_best"] >= 4.5 else 1 if candidate["odds_best"] >= 3.0 else 0
    score -= 1 if candidate["market"] == "h2h" and "draw" in candidate["selection"].lower() else 0
    label = "A" if score >= 6 else "B" if score >= 4 else "C" if score >= 2 else "D"
    return {"grade": label, "score": score, "book_count": candidate["book_count"], "odds_age_minutes": candidate["odds_age_minutes"], "implied_prob": implied, "ev_raw": ev_raw, "ev_calibrated": ev_calibrated, "ci_low": ci_low, "data_quality": data_quality}

def sort_results(results):
    dq_rank = {"HIGH": 3, "MEDIUM": 2, "LOW": 1}
    return sorted(results, key=lambda x: (CLASS_ORDER.get(x["decision"], 0), x["scorecard"]["grade"], x["ev_calibrated"], x["ci_low"], x["book_count"], dq_rank.get(x["data_quality"], 0)), reverse=True)
...
def best_per_match(results):
    ...
    for _, items in grouped.items():
        best = sorted(items, key=lambda x: (CLASS_ORDER.get(x["decision"], 0), x["scorecard"]["score"], x["ev_calibrated"], x["ci_low"]), reverse=True)[0]
        final.append(best)
...
def format_summary(summary):
    ...
    for item in summary["results"][:10]:
        if item["decision"] == "PASS":
            ...
            lines.append(item["decision"] + " | " + item["match"] + " | " + item["selection"] + " | odds " + str(item["best_odds"]) + " | stake " + str(item["stake"]) + " | EV " + str(item["ev_calibrated"]) + " | CI " + str(item["ci_low"]) + " | grade " + item["scorecard"]["grade"])
    return "\n".join(lines)
...
def format_match_report(summary):
    ...
    for best in picks[:10]:
        if best["decision"] == "PASS":
            ...
            lines.append("- " + best["match"] + " -> " + best["decision"] + " | " + best["selection"] + " | odds " + str(best["best_odds"]) + " | stake " + str(best["stake"]) + " | grade " + best["scorecard"]["grade"])
    return "\n".join(lines)
...
def format_top_picks(summary):
    ...
    for item in accepted[:5]:
        lines.append("- " + item["match"] + " | " + item["decision"] + " | " + item["selection"] + " | odds " + str(item["best_odds"]) + " | stake " + str(item["stake"]) + " | grade " + item["scorecard"]["grade"])
    return "\n".join(lines)
...
def write_report(summary):
    ...
    for idx, item in enumerate(summary["results"], start=1):
        rows.extend([str(idx) + ") " + item["match"], "Sport: " + item["sport"], "Selection: " + item["selection"], "Odds: " + str(item["best_odds"]) + " | Avg: " + str(item["avg_odds"]), "Bookmaker: " + item["bookmaker"] + " | Books: " + str(item["book_count"]), "ModelProb: " + str(item["model_prob"]) + " | Implied: " + str(item["implied_prob"]), "EV raw: " + str(item["ev_raw"]) + " | EV calibrated: " + str(item["ev_calibrated"]) + " | CI low: " + str(item["ci_low"]), "Decision: " + item["decision"] + " | Stake: " + str(item["stake"]) + " | Risk cap: " + str(item["risk_cap"]), "Scorecard: grade " + item["scorecard"]["grade"] + " | score " + str(item["scorecard"]["score"]), "Reasons: " + ", ".join(item["reasons"]), ""])
...
def format_global_summary():
    ...
    watchlist = load_watchlist()
    return "\n".join(["Runs: " + str(len(runs)), "Accepted total: " + str(sum(int(item.get("accepted_count", 0)) for item in runs)), "Rejected total: " + str(sum(int(item.get("rejected_count", 0)) for item in runs)), "Watchlist teams: " + str(len(watchlist))])
