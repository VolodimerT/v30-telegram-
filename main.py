...
def estimate_model_prob(best_odds, avg_odds, book_count, market, selection):
    ...
    gap_bonus = min(price_gap * 8.5, 2.8)
    consensus_bonus = 1.4 if book_count >= 6 else 1.0 if book_count >= 4 else 0.5 if book_count >= 3 else 0.0
    ...
    if market == "h2h" and "draw" in low_sel:
        prob -= 1.4
    if best_odds >= 3.0:
        prob -= min((best_odds - 3.0) * 1.5, 3.4)
    if best_odds >= 4.5:
        prob -= 1.0
...
def class_from_ev(ev_calibrated, ci_low, data_quality, odds):
    if ci_low < 0:
        return "PASS"
    if odds >= 4.8:
        return "MICRO" if ev_calibrated >= 8 and ci_low >= 0.5 else "PASS"
...
def build_scorecard(candidate, implied, ev_raw, ev_calibrated, ci_low, data_quality):
    ...
    score -= 1 if candidate["odds_best"] >= 4.8 else 0
...
def finalize_candidate(candidate, request_data, run_id):
    ...
    if candidate["commence_time"] < now:
        ...
        if candidate["odds_best"] >= 4.8 and CLASS_ORDER[decision] > CLASS_ORDER["MICRO"]:
            decision = "MICRO"
