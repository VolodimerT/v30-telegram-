...
SPORT_LIMITS = {
    ...
    "laliga": {"min_book_count": 3, "max_odds_age": 180},
}
    ...
    for i, token in enumerate(parts):
        if token == "mode" and i + 1 < len(parts):
            ...
            if custom in MODE_RULES:
                mode = custom
    return {"raw_text": text, "sports": sports, "strict": strict, "bank": bank, "mode": mode, "markets": ["h2h", "spreads", "totals"], "max_candidates": max_candidates}
                ...
                for market in bookmaker.get("markets", []):
                    ...
                    for outcome in market.get("outcomes", []):
                        ...
                        if group_key not in grouped:
                            ...
                            grouped[group_key] = {"match": str(away_team) + " vs " + str(home_team), "sport": sport_alias, "market": market_key, "selection": selection_text, "point": point, "commence_time": commence_time, "prices": [], "best_odds": price, "best_bookmaker": bookmaker_title, "best_odds_age_minutes": odds_age_minutes, "source_tier": source_tier, "max_class": max_class}
...
def format_summary(summary):
    lines = ["Run: " + str(summary["run_id"]), "Mode: " + str(summary["request"]["mode"]), "Status: " + str(summary["message"]), "Candidates: " + str(summary["candidates_count"]), "Accepted: " + str(summary["accepted_count"]), "Rejected: " + str(summary["rejected_count"]), ""]
    for item in summary["results"][:7]:
        if item["decision"] == "PASS":
            lines.append("PASS | " + item["match"] + " | " + ", ".join(item["reasons"]))
        else:
            lines.append(item["decision"] + " | " + item["match"] + " | " + item["sport"] + " | odds " + str(item["best_odds"]) + " | stake " + str(item["stake"]) + " | EV " + str(item["ev_calibrated"]) + " | CI " + str(item["ci_low"]))
    return "\n".join(lines)

def write_report(summary):
    file_path = REPORTS_DIR / (str(datetime.now().date()) + "_" + summary["run_id"] + "_report.txt")
    rows = ["DAILY REPORT | " + str(datetime.now().date()), "Request: " + str(summary["request"]["raw_text"]), "Mode: " + str(summary["request"]["mode"]), "Candidates: " + str(summary["candidates_count"]), "Accepted: " + str(summary["accepted_count"]), "Rejected: " + str(summary["rejected_count"]), "Status: " + str(summary["message"]), ""]
...
def format_global_summary():
    ...
    by_sport = {}
    for item in runs:
        req = str(item.get("request", "")).lower()
        for sport in SPORT_MAP.keys():
            if sport in req:
                by_sport[sport] = by_sport.get(sport, 0) + 1
    lines = ["Runs: " + str(total_runs), "Accepted total: " + str(total_accepted), "Rejected total: " + str(total_rejected), "", "Sports:"]
    if by_sport:
        for sport, count in sorted(by_sport.items(), key=lambda x: x[1], reverse=True):
            lines.append(sport + ": " + str(count))
    else:
        lines.append("no sport stats")
    return "\n".join(lines)
...
def run_auto_pipeline(request_text, dry_run=False):
    ...
    summary = {"run_id": run_id, "request": request_data, "generated_at": datetime.now(UTC).isoformat(), "candidates_count": len(results), "accepted_count": len(accepted), "rejected_count": len(results) - len(accepted), "status": "OK" if results else "NO_CANDIDATES", "message": "NO BETS / ALL PASS" if results and not accepted else "OK", "results": results}
    if not dry_run:
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await reply_long(update.message, "/auto today wnba strict | /dryrun today wnba strict | /report | /audit | /latest | /history | /summary")
async def auto_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    args = " ".join(context.args).strip()
    try:
        await reply_long(update.message, format_summary(run_auto_pipeline("AUTO " + args if args else "AUTO today basketball strict", dry_run=False)))
...
async def dryrun_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    args = " ".join(context.args).strip()
    try:
        await reply_long(update.message, "[DRYRUN]\n" + format_summary(run_auto_pipeline("AUTO " + args if args else "AUTO today basketball strict", dry_run=True)))
    except Exception as e:
        await reply_long(update.message, "ERROR_REPORT: " + type(e).__name__ + ": " + str(e))
