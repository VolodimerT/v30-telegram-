...
# League-tuning: мультипликатор к калибровочному фактору
LEAGUE_TUNING = {
...
# Слабый слейт: если 0 accepted — бот явно пишет "no plays today"
...
# ── BANKSET / MAX BETS / SAME MATCH / VALIDATE ───────────────────────────────

def set_bank(new_balance: float) -> dict:
    """Устанавливает баланс банка вручную."""
    bank = load_bank()
...
def count_today_bets() -> int:
    """Считает количество принятых ставок за сегодня."""
...
def is_same_match_blocked(match: str, market: str) -> bool:
    """Блокирует второй вход в тот же матч по тому же рынку сегодня."""
    today = datetime.now(UTC).date().isoformat()
    history = load_pick_history()
    for x in history:
        if (
            x.get("status") == "OPEN"
            and str(x.get("generated_at", ""))[:10] == today
            and x.get("match") == match
            and x.get("market") == market
        ):
            return True
    return False

def validate_config() -> list[str]:
    """Проверяет конфиг и возвращает список проблем."""
    issues = []
    bank = load_bank()
    ...
    # банк
    if balance <= 0:
        issues.append("BANK_ZERO_OR_NEGATIVE: balance=" + str(balance))
    if balance < 100:
        issues.append("BANK_CRITICALLY_LOW: balance=" + str(balance))

    # API ключ
    api_key = os.getenv("ODDS_API_KEY", "")
    if not api_key:
        issues.append("ODDS_API_KEY_MISSING: env var not set")
    elif len(api_key) < 10:
        issues.append("ODDS_API_KEY_SUSPICIOUS: key too short")

    # режимы
    mode = detect_mode(balance)
    rules = MODE_RULES.get(mode)
    if not rules:
        issues.append("MODE_RULES_MISSING: mode=" + mode)
    else:
        for field in ["min_ev", "micro", "support", "core"]:
            if field not in rules:
                issues.append("MODE_RULE_FIELD_MISSING: " + field + " in " + mode)

    # спорт-лимиты
    for sport, limits in SPORT_LIMITS.items():
        if limits.get("min_book_count", 0) < 1:
            issues.append("SPORT_LIMIT_BAD: min_book_count for " + sport)
        if limits.get("max_odds_age", 0) < 1:
            issues.append("SPORT_LIMIT_BAD: max_odds_age for " + sport)

    # league tuning
    for league, markets in LEAGUE_TUNING.items():
        for mkt, factor in markets.items():
            if not (0.3 <= factor <= 1.5):
                issues.append("LEAGUE_TUNING_SUSPICIOUS: " + league + "/" + mkt + "=" + str(factor))

    # daily exposure
    exp = load_daily_exposure()
    limit = daily_exposure_limit(balance, mode)
    if exp.get("exposed", 0.0) > limit:
        issues.append("DAILY_EXPOSURE_EXCEEDED: exposed=" + str(exp.get("exposed")) + " limit=" + str(limit))

    # max bets
    today_count = count_today_bets()
    max_bets = MAX_BETS_PER_DAY.get(mode, 5)
    if today_count >= max_bets:
        issues.append("MAX_BETS_REACHED: today=" + str(today_count) + " limit=" + str(max_bets))
...
def format_validate_report() -> str:
    ...
    lines = [
        ...
        f"Bets today: {today_count} / {max_bets}",
        f"Exposure:  {exp.get('exposed', 0.0)} / {limit}",
        "",
    ]
    if not issues:
        lines.append("✅ All checks passed — no issues found.")
    else:
        lines.append(f"⚠️ Found {len(issues)} issue(s):")
        for issue in issues:
            lines.append("  - " + issue)
    return "\n".join(lines)
...
def load_bank() -> dict:
    """Загружает виртуальный банк. Дефолт: 1000."""
...
def daily_exposure_limit(balance: float, mode: str) -> float:
    """Дневной лимит экспозиции в зависимости от режима."""
    rates = {"FROZEN": 0.02, "EMERGENCY": 0.04, "NORMAL": 0.06, "GROWTH": 0.08}
    return round(balance * rates.get(mode, 0.06), 2)

def check_daily_exposure(stake: float, balance: float, mode: str) -> tuple[bool, float, float]:
    """
    Проверяет, не превышен ли дневной лимит.
    Возвращает (allowed, current_exposed, limit).
    """
    exp = load_daily_exposure()
    limit = daily_exposure_limit(balance, mode)
    allowed = (exp["exposed"] + stake) <= limit
    return allowed, round(exp["exposed"], 2), round(limit, 2)

def register_exposure(stake: float):
    """Прибавляет ставку к дневной экспозиции."""
    exp = load_daily_exposure()
    exp["exposed"] = round(exp.get("exposed", 0.0) + stake, 2)
    save_daily_exposure(exp)

def update_bank_after_settle(pick_id: str, pnl: float):
    """Обновляет банк после расчёта ставки."""
    bank = load_bank()
...
def format_risk_report() -> str:
    ...
    lines = [
        ...
        f"Drawdown:    {drawdown}%",
        f"Mode:        {mode}",
        f"Today exp:   {today_exp} / {limit} (limit {round(limit/balance*100,1)}% of bank)",
...
def get_league_factor(sport_key: str, market: str) -> float:
    """Возвращает league-tuning мультипликатор."""
...
async def bankset_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await reply_long(update.message, "Ошибка: введи число, например /bankset 1200.50")
