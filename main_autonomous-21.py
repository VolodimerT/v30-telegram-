...
class Storage:
    ...
    async def tg(self, msg):
        logger.info(f"🔵 TG: Preparing message (app={self.app is not None}, CHAT_ID={CHAT_ID})")
        if not self.app:
            logger.error(f"🔴 TG FAILED: app not initialized")
            return
        if not CHAT_ID:
            logger.error(f"🔴 TG FAILED: CHAT_ID not set")
            return
        try:
            logger.info(f"🔵 TG: Sending to chat {CHAT_ID}...")
            await self.app.bot.send_message(
                chat_id=CHAT_ID, text=msg, parse_mode="Markdown"
            )
            logger.info(f"✅ TG: Message sent successfully")
        except Exception as e:
            logger.error(f"🔴 TG ERROR: {type(e).__name__}: {e}", exc_info=True)
