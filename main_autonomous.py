"""
main_autonomous.py - BETTING BOT (FINAL FIXED VERSION)
"""
import os
import json
import logging
...
async def scan():
    ...
    # LIMIT to 50 picks to avoid timeout
    picks = picks[:50]
    ...
    try:
        ...
        for pick in enriched.get("enriched_picks", []):
            # FILTER: Only ACCEPT/RECONSIDER
            if pick.get("hermes_recommendation") not in ["ACCEPT", "RECONSIDER"]:
                continue
            
            conf = pick.get("hermes_confidence", 0.65)
            emoji = "✅" if conf > 0.7 else "⚠️"
            recommendation = pick.get("hermes_recommendation")
