"""
main_autonomous_hermes.py — BETTING BOT v7.0.1 DEBUG+FIX
...
async def scan():
    global ALL_MATCHES
    if not ALL_MATCHES:
        await storage.tg("⚠️ No match data. Run /refresh first.", reply_markup=kb_quick())
        return
    if hermes.is_paused():
        await storage.tg("🔴 *HERMÈS STOP-LOSS ACTIVE* — scanning paused.\n"
                         "Use /hermes\\_status for details.", reply_markup=kb_quick())
        return
    ...
    if not raw:
        await storage.tg("⚠️ 0 value picks after Hermès filters.", reply_markup=kb_quick())
        return

    picks = sorted(raw, key=lambda p: p["ev"] + MK_BONUS.get(p["mk"],0),
                   reverse=True)[:TOP_N]
    mk_counter: dict = {}
    for p in picks: mk_counter[p["mk"]] = mk_counter.get(p["mk"],0) + 1

    logger.info("Scan found %d picks — sending with inline keyboards", len(picks))

    for p in picks:
        opt = int(round(bet_size(storage.bank, p["kf"])))
        text = pick_text(p, opt)
        keyboard = kb_pick(0, opt)   # pick_id=0 placeholder; replaced after send
        ...
        if sent:
            ...
            if len(_prev_stack) > _PREV_MAX: _prev_stack.pop(0)
            # Update keyboard with real pick_id (edit is cheap)
            try:
                await sent.edit_reply_markup(reply_markup=kb_pick(pick_id, opt))
                log_kb.debug("kb_pick edited OK pick_id=%s", pick_id)
            except Exception as e:
                logger.warning("edit_reply_markup failed (non-fatal): %s", e)
        else:
            # Graceful fallback — bot.send_message already retried without markup
            logger.error("Pick send failed entirely for: %s", p["match"])

        await asyncio.sleep(0.20)

    summary = " | ".join(f"{k}:{v}" for k,v in sorted(mk_counter.items()))
    await storage.tg(f"📊 *Scan done*: {len(picks)} picks\n{summary}", reply_markup=kb_quick())
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("q:"):
        ...
        if action == "stats":
            ...
            if not cand:
                await query.answer("No previous picks in this session", show_alert=True)
                return
            prev_id = cand[0]
            p = _pick_cache.get(prev_id)
            if not p:
                await query.answer("Previous pick expired", show_alert=True)
                return
    ...
    if data.startswith("accept:"):
        ...
        if not p:
            await query.answer("Pick expired — rescan", show_alert=True)
            return
    ...
    if data.startswith("custom:"):
        ...
        p = _pick_cache.get(pick_id)
        if not p:
            await query.answer("Pick expired", show_alert=True)
            return
        _pending_custom[chat_id] = pick_id
        await query.message.reply_text(
            "💰 *Custom stake*\n\nReply with the amount in UAH (e.g. `125`):",
            parse_mode="Markdown")
        return
    ...
    if data.startswith("demo_pend:"):
        await query.answer("Marked pending — use /settle later", show_alert=False)
        return
...
async def _do_accept(message, p: dict, stake: float, pick_id: int):
    """Register bet and reply with confirmation (+demo settle if DEMO_MODE)."""
    bet = storage.add_bet(
    ...
    settle_hint = ("_Demo mode — use buttons to settle:_"
                   if DEMO_MODE
    ...
    log_kb.debug("_do_accept sending confirmation bet_id=%s DEMO=%s", bet["id"], DEMO_MODE)
    try:
...
async def cmd_start(u, c):
    await u.message.reply_text(
        "🤖 *BETTING BOT v7.0.1 + HERMÈS*\n\nInline buttons active. /help",
        parse_mode="Markdown", reply_markup=kb_quick())
...
async def post_init(app: Application):
    global ALL_MATCHES
    storage.set_app(app)
    logger.info("=== BOT v7.0.1 STARTING === DEMO_MODE=%s CHAT_ID=%s", DEMO_MODE, CHAT_ID)
    ALL_MATCHES = await fetch_all_matches()
    ...
    await scan()
    await storage.tg("🤖 *Bot v7.0.1 + Hermès + Inline Buttons READY*\n\n"
                     + hermes.format_status(), reply_markup=kb_quick())
