...
# ─── PENDING BETS: msg_id -> pick info ──────────────────────────────────
_pending: dict = {}


def _kb_pick(mid, stake):
    return InlineKeyboardMarkup([[
        InlineKeyboardButton(
            'YES  ' + str(int(stake)) + ' UAH',
            callback_data='acc:' + str(mid) + ':' + str(int(stake))
        ),
        InlineKeyboardButton('SKIP', callback_data='skip:' + str(mid)),
    ]])


def _kb_settle(bet_id):
    return InlineKeyboardMarkup([[
        InlineKeyboardButton('WIN',     callback_data='win:'  + str(bet_id)),
        InlineKeyboardButton('LOSS',    callback_data='loss:' + str(bet_id)),
        InlineKeyboardButton('PENDING', callback_data='pend:' + str(bet_id)),
    ]])
...
async def scan():
    ...
    try:
        ...
        for idx, pick in enumerate(enriched_picks):
            ...
            logger.info(f"Pick {idx+1}: {match} | {recommendation} | conf={conf:.0%} | odds={odds}")
            
            # ── ALWAYS: odds + kelly gates ──────────────────────────
            if not (MIN_ODDS <= odds <= MAX_ODDS):
                logger.info(f"  ❌ Skip odds {odds} outside [{MIN_ODDS},{MAX_ODDS}]")
                filtered += 1
                continue
            ...
            if kelly_fraction < MIN_KELLY:
                logger.info(f"  ❌ Skip Kelly {kelly_fraction*100:.1f}% < {MIN_KELLY*100:.1f}%")
                filtered += 1
                continue

            # ── DUAL PATH ────────────────────────────────────────────
            hermes_known = recommendation in ("ACCEPT", "RECONSIDER")
            if not hermes_known:
                # Path A: new match — Hermès untrained, use EV gate
                if ev < 0.05 or conf < 0.50:
                    logger.info(f"  ❌ Skip new match EV={ev:.3f} conf={conf:.0%}")
                    filtered += 1
                    continue
                rec_label = "ACCEPT (system)"
                emoji = "🤖"
                logger.info(f"  🤖 Path A EV={ev:.3f} Kelly={kelly_fraction*100:.1f}%")
            else:
                # Path B: Hermès has opinion — confidence gate
                if conf < MIN_CONFIDENCE:
                    logger.info(f"  ❌ Skip hermes conf={conf:.0%} < {MIN_CONFIDENCE:.0%}")
                    filtered += 1
                    continue
                rec_label = recommendation
                emoji = "✅"
                logger.info(f"  ✅ Path B {recommendation} conf={conf:.0%} Kelly={kelly_fraction*100:.1f}%")
            ...
            # ── Send pick card with inline buttons ──
            if storage.app and CHAT_ID:
                try:
                    sent_msg = await storage.app.bot.send_message(
                        chat_id=CHAT_ID,
                        text=msg,
                        parse_mode='Markdown',
                        reply_markup=_kb_pick(0, optimal_stake)
                    )
                    real_mid = sent_msg.message_id
                    _pending[real_mid] = {
                        'match': pick['match'],
                    ...
                    await sent_msg.edit_reply_markup(
                        reply_markup=_kb_pick(real_mid, optimal_stake)
                    )
                except Exception as _e:
                    logger.error('Card send error: ' + str(_e))
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith('acc:'):
        ...
        if not info:
            await q.message.reply_text('Pick expired - run /scan to refresh.')
            return
        try:
            await q.message.edit_reply_markup(
                reply_markup=_kb_done('Accepted ' + str(int(stake)) + ' UAH')
            )
        except Exception:
            pass
        bet    = storage.add_bet(info['match'], info['selection'], info['odds'], stake)
        bet_id = bet['id']
        txt = ('Bet #' + str(bet_id) + ' registered\n'
               + info['match'] + '\n' + info['selection'] + '\n'
               + 'Odds: ' + str(info['odds'])
               + '  Stake: ' + str(int(stake)) + ' UAH\n\nSettle when done:')
        await q.message.reply_text(txt, reply_markup=_kb_settle(bet_id))
        return
    ...
    if data.startswith('win:'):
        ...
        bet = next((b for b in storage.bets if b['id'] == bet_id), None)
        if not bet:
        ...
        storage.results.append({
            'bet_id': bet_id, 'match': bet['match'],
            'result': 'WON', 'stake': bet['stake'], 'profit': profit,
        ...
        try:
            await q.message.edit_reply_markup(
                reply_markup=_kb_done('WIN  +' + str(profit) + ' UAH')
            )
        except Exception:
            pass
        await q.message.reply_text(
            'WIN\n' + bet['match']
            + '\nProfit: +' + str(profit) + ' UAH'
            + '\nBank: ' + str(round(storage.bank, 2)) + ' UAH'
        )
        return
    if data.startswith('loss:'):
        ...
        storage.results.append({
            'bet_id': bet_id, 'match': bet['match'],
            'result': 'LOST', 'stake': bet['stake'], 'profit': loss,
            'settled_at': datetime.now(UTC).isoformat(),
        })
        bet['status'] = 'SETTLED'
        storage.save()
        try:
            await q.message.edit_reply_markup(
                reply_markup=_kb_done('LOSS  ' + str(loss) + ' UAH')
            )
        except Exception:
            pass
        await q.message.reply_text(
    ...
    if data.startswith('pend:'):
        try:
            ...
            pass
        await q.message.reply_text('Pending. Use /bets to settle later.')
        return
