...
async def fetch_picks() -> list:
    ...
    all_picks = []
    market_seen = {"h2h": 0, "spreads": 0, "totals": 0}
    sport_market_seen = {}
        ...
        for sport_key in active:
            ...
            for event in events:
                ...
                for market in bks[0].get("markets", []):
                    mk = market.get("key", "")
                    market_seen[mk] = market_seen.get(mk, 0) + 1
                    smk = f"{sport_key}:{mk}"
                    sport_market_seen[smk] = sport_market_seen.get(smk, 0) + 1
    ...
    logger.info("Market presence in API response: " + ", ".join(f"{k}={v}" for k, v in market_seen.items()))
    top_sports = sorted(sport_market_seen.items(), key=lambda x: x[1], reverse=True)[:20]
    if top_sports:
        logger.info("Top sport:market counts: " + ", ".join(f"{k}={v}" for k, v in top_sports))
    return all_picks
...
async def scan(app=None):
    ...
    for i, p in enumerate(raw, start=1):
        ...
        if ok:
            ...
            passed.append(p)
            if i <= 25:
                logger.info(f"  ✓ Pick {i}: {p['market']} odds={p['odds']:.2f} conf={p['conf']:.0%} ev={ev:+.3f} kelly={kelly*100:.1f}%")
        else:
            ...
            if i <= 25:
                logger.info(f"  ✗ Pick {i}: {p['market']} odds={p['odds']:.2f} conf={p['conf']:.0%} ev={p['ev']:+.3f} kelly={p['kelly']*100:.1f}% → {reason}")
    ...
    if not passed:
        ...
        market_line = ", ".join(f"{k}={v}" for k, v in sorted(market_counts.items())) or "none"
        msg = (
            f"🔍 Scan: 0 picks passed\n"
            f"Fetched {len(raw)} | Markets: {market_line}\n"
            f"Odds:{reject_odds} Conf:{reject_conf} EV:{reject_ev} Kelly:{reject_kelly} filtered\n"
            f"Bank: {_S['bank']:.0f} UAH"
        )
        if app and CHAT_ID:
