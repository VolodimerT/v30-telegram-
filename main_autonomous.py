"""
main_autonomous.py - BETTING BOT WITH CORRECT ODDS API
"""
import os
import json
import logging
...
async def fetch_real_matches():
    """Fetch real matches from Odds API - CORRECTED"""
    try:
            ...
            for sport in sports[:5]:
                ...
                if odds_response.status_code == 200:
                    ...
                    if events and len(events) > 0:
                        processed = []
                        for event in events:
                            # CORRECTED: Use home_team/away_team instead of "match"
                            ...
                            processed.append({
                                ...
                                "home": home,
                                "away": away,
                                "markets": event.get("bookmakers", [{}])[0].get("markets", {}) if event.get("bookmakers") else {}
                            })
                        matches[sport_key] = processed
                        logger.info(f"Got {len(processed)} matches for {sport_key}")
...
async def scan():
    ...
    for league, matches in ALL_MATCHES.items():
        for match in matches:
            # FIXED: Parse API format correctly
            sport = match.get("sport", "")
            home = match.get("home", "Team1")
            away = match.get("away", "Team2")
            match_name = match.get("match", f"{home} vs {away}")
            
            # Get bookmakers data from API
            bookmakers = match.get("bookmakers", [])
            if not bookmakers or not bookmakers[0]:
                continue
            ...
            for market in markets_data:
                ...
                # Get best odds from outcomes
                best_odds = max([o.get("price", 1.0) for o in outcomes])
                ...
                picks.append(pick)
                logger.info(f"DEBUG: {match_name} | {market_key} | {best_odds}")
    
    logger.info(f"Analyzing {len(picks)} picks...")
    if len(picks) == 0:
        logger.warning("No picks generated from matches")
        return
    ...
    try:
        ...
        for pick in enriched.get("enriched_picks", []):
            logger.info(f"DEBUG: {pick.get('match')}: {pick.get('hermes_recommendation')} ({pick.get('hermes_confidence')})")
