"""
main_autonomous.py - BETTING BOT (FIXED)
"""
import os
import json
import logging
...
async def scan():
    ...
    try:
        ...
        for pick in enriched.get("enriched_picks", []):
            ...
            # SEND ALL PICKS - no filter
            storage.add_rec(pick["match"], pick["selection"], pick["odds"], conf, recommendation)
            msg = f"{emoji} {pick['sport'].upper()}\n\n{pick['match']}\n{pick['selection']}\n{pick['odds']}\nHermès: {recommendation} ({conf:.0%})\n\n/place_bet \"{pick['match']}\" \"{pick['selection']}\" {pick['odds']} 50"
            await storage.tg(msg)
            await asyncio.sleep(0.1)  # Delay to avoid rate limit
            sent += 1
        logger.info(f"Sent {sent} picks from Hermès")
    except Exception as e:
