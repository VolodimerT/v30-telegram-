...
def format_latest():
    runs = read_runs()
    if not runs:
        return "Пока нет latest"
    item = runs[-1]
    lines = []
    lines.append("Run: " + str(item.get("run_id", "")))
    lines.append("Generated: " + str(item.get("generated_at", "")))
    lines.append("Request: " + str(item.get("request", "")))
    lines.append("Accepted: " + str(item.get("accepted_count", 0)))
    lines.append("Rejected: " + str(item.get("rejected_count", 0)))
    lines.append("Report: " + str(item.get("report_path", "")))
    return "\n".join(lines)
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = "/auto today wnba strict | /dryrun today wnba strict | /report | /audit | /history | /summary | /latest"
